    ...
    <div ref={containerRef}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12, marginTop: 16 }}>
        ...
        <MetricExplainer label="CV(RMSE)" value={`${cvRMSE.toFixed(1)}%`} color={C.amber}
          explanation="RMSE divided by the mean, expressed as a percentage. This normalizes the error so you can compare across buildings of different sizes. A widely used threshold is ≤15% for monthly models." />
        <MetricExplainer label="NMBE" value={`${NMBE.toFixed(2)}%`} color={C.orange}
          explanation="Normalized Mean Bias Error. Measures whether the model systematically over- or under-predicts. Near zero means unbiased. A widely used threshold is ±5% for monthly models." />
      </div>
...
function LiveMetric({ label, value, color, tag, threshold }) {
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, padding: "10px 14px" }}>
      ...
      {threshold && <div style={{ fontSize: 9, color: C.textDim, fontFamily: "'IBM Plex Sans'", marginTop: 2 }}>Threshold: {threshold}</div>}
    </div>
  );
}
