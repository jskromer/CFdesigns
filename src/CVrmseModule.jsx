...
export default function CVrmseModule() {
  ...
  return (
    <div ref={containerRef} style={{ maxWidth: 1100, margin: "0 auto", fontFamily: "'IBM Plex Mono', monospace" }}>
      ...
      <P>CV(RMSE) is the single most important metric in M&V model validation. A widely used threshold is 15% for monthly models. But what does it actually <Em>measure</Em>? And why can the same model quality produce completely different CV(RMSE) values?</P>
      ...
      {section === 1 && (
        <Panel>
          ...
          <Box type="concept" title="1. Building size matters">
            <P style={{ marginTop: 0 }}>Large buildings (high ȳ) will almost always pass the 15% CV(RMSE) threshold more easily — not because the model fits better, but because the denominator is larger. A 50,000 sq ft office and a 5,000 sq ft branch might have the same RMSE relative to their weather-dependent load, but very different CV(RMSE) values.</P>
          </Box>
          ...
          <Box type="warning" title="4. The 15% threshold is a floor, not a ceiling">
